...
double Entry_Amount = 0.20; // Entry lots
double progressiveLot[100];

input int Stop_Loss   = 2800; // Stop Loss (pips)
input int Take_Profit = 2100; // Take Profit (pips)

input int Normalised_Stop_Loss   = 280;
input int Normalised_Take_Profit = 210;

double FirstEntryPrice = 0.0;
double SecondEntryPrice = 0.0;
double nextEntryPrice = 0.0;
double previousEntryPrice = 0.0;
int SecondEntryPriceDeviation = 700;
...
input int Ind0Param0 = 30; // %K Period 30
input int Ind0Param1 = 70; // %D Period 70
input int Ind0Param2 = 3; // Slowing 3
...
{ 
   ...
   if (positionModified){
      ...
      if (lastCommand == OP_BUY_STOP){
         if(tick.ask <= SecondEntryTakeProfit || tick.bid <= SecondEntryTakeProfit || tick.ask >= SecondEntryStopLoss || tick.bid >= SecondEntryStopLoss){
            CloseAllPosition();
         }
...
void CloseAllPosition()
 ...
 posType   = OP_FLAT;
   ...
   for(int posIndex=0;posIndex<posTotal;posIndex++)
     {
      ...
      if(PositionSelectByTicket(ticket) &&
         ...
         )
        {
         ...
         const int command=commandSelect(posType);
         ManageOrderSendClose(command,posLots,0,0,posTicket);
         break;
        }
   
  }
 }
  int commandSelect(double posiType){
      if (posType == OP_BUY){
         return ORDER_TYPE_BUY;
      }else if (posType == OP_SELL){
         return ORDER_TYPE_SELL;
      }else if (posType == OP_BUY_STOP){
  ...
  void ManageOrderSendClose(int command,double lots,double stopLoss,double takeProfit,ulong ticket)
  {
...
void ManageOrderSend(int command,double lots, ulong ticket)
      ...
      if(IsTradeContextFree())
        {
         ...
         if(isOrderCheck && isOrderSend && result.retcode==TRADE_RETCODE_DONE)
            ...
            Buy_Profit = Normalised_Take_Profit * Entry_Amount;
            Buy_Loss = Normalised_Stop_Loss * Entry_Amount;
            Entry_Amount = 0.24;
      ...
      if(IsTradeContextFree())
        {
         ...
         if(isOrderCheck && isOrderSend && result.retcode==TRADE_RETCODE_DONE)
            ...
            if (command==OP_BUY_STOP){
               ...
               Sell_Profit += Normalised_Take_Profit * Entry_Amount;
               Sell_Loss += Normalised_Stop_Loss * Entry_Amount;
               calculateLot();
            }
            orderSuccess = true;
  ...
  void calculateLot(){
   if (lastCommand == ORDER_TYPE_BUY_STOP){
      double newBuyProfit = Buy_Profit;
      double lotIncrement = 0.02;
      double lotsIncreReal = 0.20;
      double newBuyLoss = Buy_Loss;
      double newSellLoss = Sell_Loss;
      double newSellProfit = Sell_Profit;
      
      while(newSellProfit < newBuyLoss){
         newSellProfit = Sell_Profit;
         newSellProfit += Normalised_Take_Profit * lotsIncreReal;
         lotsIncreReal += lotIncrement;
         Print(newSellProfit, lotsIncreReal);
         
      }
      Entry_Amount = lotsIncreReal;
   }else{
      double newBuyProfit = Buy_Profit;
      ...
      while(newBuyProfit < newSellLoss){
         newBuyProfit = Buy_Profit;
         newBuyProfit += Normalised_Take_Profit * lotsIncreReal;
         lotsIncreReal += lotIncrement;
         Print(newBuyProfit, lotsIncreReal);
      ...
      if(IsTradeContextFree())
        {
         ...
         if(isOrderCheck && isOrderSend && result.retcode==TRADE_RETCODE_DONE){
             ...
             if (PositionGetInteger(POSITION_TYPE) == OP_BUY){
               ...
               Sell_Profit = Normalised_Take_Profit * 0.2;
               Sell_Loss = Normalised_Stop_Loss * 0.2;
...
//void CalculateLot(){
  // int stopLoss = 280;
   //int takeProfit = 210;
   //double initialLot = 0.1;
   //double newLot = 0;
   
   //progressiveLot[0] = Entry_Amount;
   ...
   //for (int count = 0; count<=100; count++){
     // buyProfit += takeProfit * initialLot;
     // buyLoss += stopLoss * initialLot
     // ;
     // double incrementLot = 0.01;
    //  if (commands == "buy"){
    //     double newIncremento = 0;
      //   double newSellProfit = sellProfit;
     //    while(newSellProfit < buyLoss){
        //    newIncremento += incrementLot;
       //     newSellProfit = (takeProfit * newIncremento);
         //   if(newSellProfit < buyLoss){
         //      newSellProfit = sellProfit;
        //    }
        // }
        // sellProfit += newSellProfit;
        // sellLoss += stopLoss * newIncremento;
        // progressiveLot[count + 1] = newIncremento;
        // Print(newIncremento);
       //  commands = "sell";
     // }else{
      //   double newIncremento=0;
      //   double newBuyProfit = buyProfit;
       //  while(newBuyProfit < sellLoss){
       //     newIncremento += incrementLot;
          //  newBuyProfit = (takeProfit * newIncremento);
          //  if(newBuyProfit < sellLoss){
          //     newBuyProfit = buyProfit;
          //  }
         //}
        // buyProfit += newBuyProfit;
        // buyLoss += stopLoss * newIncremento;
       //  progressiveLot[count + 1] = newIncremento;
        // Print(newIncremento);
        // commands = "buy";
     // }
